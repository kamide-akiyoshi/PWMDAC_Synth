...
●楽器を作ろうとして、こんな問題にぶち当たったことはありませんか？
...
このライブラリは、一定の周期でタイマー割り込みを発生させ、
出したい音の波形に合わせてPWMのパルス幅を毎回更新する
ことによって色々な波形の音を鳴らします。
...
展開した PWMDAC_Synth フォルダを

	\Program Files\arduino-<バージョン番号>\libraries\
...
そのまま PC の LINE IN などにつないでも、一応音は聞こえますが、
...
また、CAmiDion など PWMDAC_Synth を使っているスケッチも参考にしてください。
...
・MIDIチャンネル操作
	...
	MIDIチャンネルごとの操作には、MIDIチャンネルへのポインタが必要になります。

	ただし、NOTE OFF、NOTE ON については VoiceStatus クラスによる
	発声管理が必要なため、MidiChannel クラスには実装していません。
	PWM_SYNTH にある noteOff()、noteOn() を使ってください。

	ピッチベンドについても PWM_SYNTH.pitchBend() を使うように
	してください。VoiceStatus も一緒に更新され、現在鳴っている音にも
	きちんと反映されます。
	MidiChannel.setPitchBend() で更新しただけでは、VoiceStatus が
	更新されないので音程をリアルタイムに変化させることができません。
...
・MIDI関数
	...
	ピッチベンドは、デフォルトの±１全音のみです。
	ピッチベンドセンシテビティ（感度を上げてベンド幅を大きくする）には
	対応していません。

	コントロールチェンジは、モジュレーション（１番）のみに反応します。
...
・音色変更
	...
	同時発音数は、#define 値を変えることで原理的にもっと増やせますが、
	あまり増やしすぎると処理が追いつかなくなったりします。
	（６重和音ぐらいが限界のようです）
...
このライブラリのダウンロード：SourceForgeの作業部屋：電子楽器CAmiDion
	http://sourceforge.jp/users/kamide/pf/CAmiDion/files/
